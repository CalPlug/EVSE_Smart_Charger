...
float b_v1 = -32; // offset voltage channel 1 previously -2.32. testing with new valuse to bypass fault
  
float m_v2 = 1.24; // gain voltage channel 2
float b_v2 = -90; // offset voltage channel 2 previously -51.8. testing with new valuse to bypass fault
...
void APsetup(void) {
  ...
  #ifdef DEBUG
  Serial.println("AP mode initialized!(main AP mode)");
  #endif
  dnsServer.start(DNS_PORT, "*", apIP);  
  server.begin();
  bool clientcomplete = false;
...
// Interrupt function for the button (ISR) - this is optimized to run fast
...
{  
  ...
  testvalue = 0; //initialize holder for new value
  for(int i = 0; i < 20; i++){ 
    testvalue += myADE7953.getVrms(); //read 20 times, this is rms not instantaneous voltage value
    delay(1); //delay between readings in the loop 
    Serial.println("Test value")
    Serial.println(testvalue);  
  } 
  testvalue = testvalue / 20.0; // this finds the average value for voltage 1
  testvalue = (testvalue*m_v1) + b_v1;
  #ifdef DEBUG
  
  Serial.println("The VRMS for multiplex LOW:"); 
  Serial.println(testvalue);  //return average value with callibration
  #endif
  digitalWrite(multiplex, HIGH);  
  delay(500);

  testvalue2 = 0; //initialize holder for new value
  for(int i = 0; i < 20; i++){ 
    testvalue2 += myADE7953.getVrms();    
  } 
  testvalue2 = testvalue2 / 20.0; // this finds the average value for voltage 2
  testvalue2 = (testvalue2 * m_v2) +b_v2;
  #ifdef DEBUG
  Serial.println("The VRMS for multiplex HIGH:");
  ...
  // delivered current to be supplied
    ...
    // WR
      ...
      client.publish("esp/response", &charge.state); 
       ...
       client.publish("esp/response", "kjhgfxdcgyuioupo0[0ipiugyghfdx");   
    }
  }         
}
