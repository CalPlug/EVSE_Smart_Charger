 ...
 *****************************************************************************/
uint8_t g_message[] ="K";  //Heartbeat message - send back heartbeat command to tablet link, this is the UART message to reply with, and is used by tablet to assess connection is still active  //FORMERLY "K\n\r";
...
int main( void )
{
	    ...
	    //Initial position
	    	set_deg_abs(40.0, JAW);  //Send JAW (JAW CLOSURE POSITION) to INTERMEDIATE position using incremental motion for safe holding
	    	set_deg_abs(120.0, JAWPIVOT);  //Send JAWPIVOT (JAW PIVOTING)to an INTERMEDIATE position using incremental motion for safe holding
	    	set_deg_abs(60.0, JAWROT);  //Send JAWROT (JAW ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
	    	...
	    	//Initial HOLD Point
	    	set_deg_abs(50.0, JAW);  //Send JAW (JAW CLOSURE POSITION) to INTERMEDIATE position using incremental motion for safe holding
	    	...
	    	set_deg_abs(70.0, JAWROT);  //Send JAWROT (JAW ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
      ...
      if ( io_state == 0x00000001 )
      	{
    	    ...
    	    for (;;) //infinite loop for demo - only stopped by resetting
    	    {
    	    	Demo(); //Demo motion routine - Start "Demo();" in infinite loop
    	    	delay (250); //delay before repeating function
    	    	io_state = GPIO_get_inputs( &g_gpio );
    	    	if (io_state!=0x00000001)
    	    	{
    	    		break;
      	...
      	{
			#ifdef VERBOSEDEBUGCONSOLE
    	  	iprintf("Switch 2 pressed - Push and hold to stop demo\n"); //this is a kludge for an ESTOP, the break will cause it to return to the main function, it's not an ESTOP
            #endif
    	    GPIO_clear_irq( &g_gpio, GPIO_0 );  //clear IRQ - Button 1
    	    GPIO_clear_irq( &g_gpio, GPIO_1 );  //clear IRQ - Button 2
    	    //disable IRQs to ignore other inputs before reset
    	  	GPIO_disable_irq( &g_gpio, GPIO_0 );//Disable interrupt to button 1 (//GPIO_disable_irq( &g_gpio, GPIO_0 );)
    	  	GPIO_disable_irq( &g_gpio, GPIO_1 );//Disable interrupt to button 2 (//GPIO_disable_irq( &g_gpio, GPIO_0 );)
    	  	//disable CorePWM (should be implemented to freeze servos)
    	  	    	  	for (;;)
    	  	    	  	{
    	  	    	  	//lock in infinite loop until reset
    	  	    	  	}
      	}
...
void motiononeHoldtoA ()
{
		...
		set_deg_inc(112.0,92.0,1,1,10,ARMROT); //pivot arm from hold to Point A)
		delay(500);
		set_deg_inc(100.0,30.0,1,1,10,ARMEXT); //brings arm down to test tube
		delay(500);
		set_deg_inc(98.0,130.0,0,1,10,JAWPIVOT); //adjusting jaw down to test tube
		delay(500);
		grabtube (3);//closes jaws
		delay(700);
		set_deg_inc(96.0, 98.0, 0 ,1,10,JAWPIVOT); //adjusting jaw down to secure hold
		delay(300);
		set_deg_inc(60.0, 100.0, 0, 1, 10, ARMEXT); // moves arm up to lift test tube
		delay(500);
}
...
void motiononeAtoB()
{
	set_deg_inc(65.0, 112.0, 0, .5, 5, ARMROT); //rotate arm from point A to point B
//	set_deg_inc(60.0, 70.0, 0, 1, 10, JAWROT);  //Send JAWROT (JAW ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
	delay(500);
	set_deg_inc(65.0,70.0, 0 , 1, 1, JAWROT);//adjusting jaw rotation to the counter-clockwise from behind the arm
	delay(200);
	set_deg_inc(80.0, 96.0, 0 ,1,10,JAWPIVOT); //adjusting jaw down to place above B
	delay(300);
	set_deg_inc(84.0, 60.0, 1, 1, 10, ARMEXT); //placing tube into position B
	delay (500);
}


void motiononebheighttransition()//to make sure jaw doesn't get caught on cap
{

	set_deg_inc(90.0, 84.0, 1, 1, 10, ARMEXT); // move arm up after dropping tube to hole B
	delay(500);
	set_deg_inc(66.0, 65.0, 1, .5, 5, ARMROT); //to move arm out of way of tube when lifting
	set_deg_inc(50.0, 90.0, 0, 1, 10, ARMEXT); // move arm up after dropping tube to hole B
	delay(500);
	set_deg_inc(60.0, 60.0, 0, 1, 10, JAWROT);  //It just works with this
	delay (500);
}
...
void motiononeBtoHold()
{
	...
	set_deg_inc(130.0, 80.0, 1, 1, 10, JAWPIVOT);  //Send JAWPIVOT (JAW PIVOTING)to an INTERMEDIATE position using incremental motion for safe holding
//	set_deg_inc(70.0, 60.0, 1, 1, 10, JAWROT);  //Send JAWROT (JAW ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
	set_deg_inc(92.0, 65.0, 1, 1, 10, ARMROT);  //Send ARMROT (ARM ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
...
void motiontwoHoldtoB()
{
	...
	set_deg_inc(65.0, 92.0, 0, 1, 10, ARMROT); //pivot arm from hold to Point B
	delay(1000);
	set_deg_inc(85.0, 30.0, 1, 1, 10, ARMEXT); //bring arm down to tube at Point B
	delay(1000);
	set_deg_inc(96.0 , 130.0, 0, 1, 10, JAWPIVOT); //adjust jaw down
	delay(1000);
	set_deg_inc(100.0, 85.0, 1, 1, 10, ARMEXT); //bring arm down to tube at Point B
	delay(700);
	grabtube(3);// close jaws
	delay(700);
	set_deg_inc(92.0, 96.0, 0, 1, 10, JAWPIVOT); //adjust jaw down to make sure it is secure
	set_deg_inc(70.0, 100.0, 0, 1, 10, ARMEXT); //moves arm to lift tube out of B
	delay(1000);
}
...
{
	delay(1000);
	set_deg_inc(112.0, 65.0, 1, 1, 5, ARMROT);//moves tube from B to A(varies from 111.0 and 112.0-no in between, one overshoots, one undershoots)
	delay(1000);
	set_deg_inc(62.0,70.0, 0, 1, 10, JAWROT); //rotates jaw to align into hole A, because ARMROT over or under shoots
	delay(500);
	set_deg_inc(80.0, 122.0, 0, 1, 10, JAWPIVOT); //moves jaw down to align tube to hole A
	delay(1000);
	set_deg_inc(79.0, 70.0, 1, 1, 10, ARMEXT); //place tube in point A
	delay(1000);
	set_deg_inc(70.0,62.0, 1, 1, 10, JAWROT); //rotates jaw because it gets caught

}

void motionotwoaheighttransition()
{
	set_deg_inc(90.0, 78.0, 1, 1, 10, ARMEXT); // move arm up after dropping tube to hole A
	set_deg_inc(109.0, 112.0, 0,1, 5, ARMROT); //gets jaw out of way of tube
	delay(200);
	set_deg_inc(50.0, 90.0, 0, 1, 10, ARMEXT); // move arm up after dropping tube to hole A
	delay(500);
}
...
void motiontwoAtoHold()
{
	set_deg_inc(110.0, 110.0, 1, 1, 10, JAW);  //Send JAW (JAW CLOSURE POSITION) to INTERMEDIATE position using incremental motion for safe holding
	set_deg_inc(130.0, 80.0, 1, 1, 10, JAWPIVOT);  //Send JAWPIVOT (JAW PIVOTING)to an INTERMEDIATE position using incremental motion for safe holding
	set_deg_inc(70.0, 62.0, 1, 1, 10, JAWROT);  //Send JAWROT (JAW ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
	set_deg_inc(92.0, 111.0, 1, 1, 10, ARMROT);  //Send ARMROT (ARM ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
	set_deg_inc(30.0, 50.0, 0, 1, 10, ARMEXT);  //Send JAWPIVOT to an INTERMEDIATE position using incremental motion for safe holding
	set_deg_abs(100.0, AUX0);  //Set AUX0 to 100 - just a number to initialize, no use at this point
	set_deg_abs(100.0, AUX1);  //Set AUX1 to 100 - just a number to initialize, no use at this point
	delay(500);
...
void motionthreeHoldtoC() //Hold to C
{
    ...
    set_deg_inc(86.0,92.0,0,.5,10,ARMROT); //Rotate arm over C
    delay(500);
    set_deg_inc(100.0,30.0,1,1,10,ARMEXT); //brings arm down to test tube
    delay(600);
    set_deg_inc(100.0,130.0,0,1,10,JAWPIVOT); //adjusting jaw down
    delay(200);
	grabtube(3);//closes jaws
	delay(700);
	set_deg_inc(80.0, 100.0, 0, 1, 15, ARMEXT);//lifts tube C
	set_deg_inc(99.0, 100.0,0,1,10,JAWPIVOT);//lifts tube C out of the way to further lift
	delay(500);
	set_deg_inc(70.0, 80.0, 0, 1, 15, ARMEXT); //moves arm to lift tube all the way out
	delay(500);
}
...
void motionthreeCtoD()
{
	set_deg_inc(48.0, 86.0, 0, 1, 10, ARMROT);//Rotate arm from C to D
	delay(600);
	set_deg_inc(79.0, 99.0, 0, 1, 10, JAWPIVOT);//Places tube above D
	delay(500);
	set_deg_inc(82.0, 70.0,1,1,15,ARMEXT);//Places tube into D
	delay(800);
...
void motionthreeDtoHold()
{
	set_deg_inc(90.0, 82.0, 1, 1, 10, ARMEXT);//Lift from D
	delay(200);
	set_deg_inc(50.0, 48.0, 1, 1, 10, ARMROT);//Return to Hold from D
	delay(200);
	set_deg_inc(30.0, 90.0, 0, 1, 10, ARMEXT);//return to hold from D
	set_deg_inc(130.0, 79.0, 1, 1, 10, JAWPIVOT);  //Send JAWPIVOT (JAW PIVOTING)to an INTERMEDIATE position using incremental motion for safe holding
	set_deg_inc(70.0, 70.0, 1, 1, 10, JAWROT);  //Send JAWROT (JAW ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
	set_deg_inc(92.0, 46.0, 1, 1, 10, ARMROT);  //Send ARMROT (ARM ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
	delay(500);
}
...
void motionfourHoldtoD()
{
	set_deg_inc(52.0, 92.0, 0, 1, 10, ARMROT);// Move from Hold to D
	delay(500);
	set_deg_inc(82.0, 130.0, 0, 1, 10, JAWPIVOT);// Move jaw pivot lower to grab tube
	delay(100);
	set_deg_inc(88.0, 30.0, 1, 1, 10, ARMEXT);// lowers arm to grab tube
	delay(600);
	grabtube(3);//closes jaw
	delay(700);
	set_deg_inc(80.0, 82.0, 0, 1, 10,JAWPIVOT); //adjusting tube within hole
	delay(500);
	set_deg_inc(65.0, 88.0, 0, 1, 10, ARMEXT); //lifting tube out of hole
	delay(500);
...
void motionfourDtoC()
{
	set_deg_inc(95.0, 52.0, 1, 1, 10, ARMROT); // move tube from D to C
	delay(500);
	set_deg_inc(71.0, 80.0, 0, 1, 10, JAWPIVOT); // Aligns tube to D
	delay(700);
	//set_deg_inc(62.0, 70.0, 0, 1, 10, JAWROT);  //Send ARMROT (ARM ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
	delay(200);
	set_deg_inc(78.0, 65.0, 1, 1, 10, ARMEXT); //puts tube into C
	delay(500);
...
void motionfourCheighttransition()
{
	...
	set_deg_inc(87.0, 78.0, 1, 1, 10, ARMEXT); // move arm down wiggle off tube
	delay(500);
	set_deg_inc(91.0, 95.0, 1, .5, 5, ARMROT); //moves arm out of the way preventing hitting cap
	delay(200);

}
void motionfourCtoHold()
{
	set_deg_inc(48.0, 65.0, 0, 1, 10, JAWROT);//moves jaw rotation so it doesn't get caught on cap
	set_deg_inc(130.0, 71.0,1,1,10,JAWPIVOT); //moves jaw pivot upwards
	delay(500);
	set_deg_inc(30.0, 87.0, 0, 1, 10, ARMEXT); //lifting back to hold
	delay(500);
	set_deg_inc(92.0, 91.0, 1, 1, 10, ARMROT); // move arm back to hold
	delay(500);
...
void motionfiveAtoagitate()
{
		...
		set_deg_inc(130.0, 100.0, 1, 1, 10, JAWPIVOT);//pulls tube out from A
		set_deg_inc(180.0, 60.0, 1, 1, 5, JAWROT); //rotate tube from hold to a mix position-clockwise
        delay(600);
        set_deg_inc(5.0, 180.0, 0, 1, 5, JAWROT); //rotate tube back from mix position on -counter-clockwise
        delay(500);
        set_deg_inc(180.0, 5.0, 1, 1, 5, JAWROT);//rotate clockwise
        delay(500);
        set_deg_inc(60.0, 180.0, 0, 1, 5, JAWROT);//rotate counter-clockwise back to hold
        delay(600);
...
void putdownA()
{
		...
		set_deg_inc(70.0,60.0, 1, 1, 10, JAWROT); //rotates jaw to align into hole A
		delay(500);
		set_deg_inc(83.0, 130.0, 0, 1, 10, JAWPIVOT); //moves jaw down to align tube to hole A
		delay(500);
		set_deg_inc(83.0, 60.0, 1, 1, 10, ARMEXT); //place tube in point A
		delay(1500);
		grabtube(2);// opens jaw
		delay(500);
		set_deg_inc(90.0, 83.0, 1, 1, 10, ARMEXT); // drop arm a bit more to leave bracket from tube
		set_deg_inc(72.0,70.0, 1, 1, 10, JAWROT); // rotate jaw a bit to avoid pulling tube back up
		set_deg_inc(50.0, 90.0, 0, 1, 10, ARMEXT); // move arm up after dropping tube to hole A
		delay(500);
...
void motionsevenCtoagitate()
{
		set_deg_inc(120.0,99.0,1,1,10,JAWPIVOT); //pick tube up
        set_deg_inc(170.0, 70.0, 1, 1, 5, JAWROT); //rotate tube
        delay(600);
        set_deg_inc(70.0, 170.0, 0, 1, 5, JAWROT); //rotate tube back
        delay(500);
        set_deg_inc(40.0, 70.0, 1, 1, 5, JAWROT); //rotate tube
        delay(500);
        set_deg_inc(70.0, 40.0, 0, 1, 5, JAWROT); //rotate tube
        delay(600);
        set_deg_inc(170.0, 70.0, 1, 1, 5, JAWROT); //rotate tube
                delay(600);
                set_deg_inc(70.0, 170.0, 0, 1, 5, JAWROT); //rotate tube back
...
void putdownC()
{
		set_deg_inc(84.0 ,120.0, 0, 1, 10, JAWPIVOT); //adjust jaw pivot
		delay(500);
        set_deg_inc(68.0,70.0, 0, 1, 5, JAWROT); //rotate as substitution to inaccurate ARMROT
        delay(500);
        ...
        delay(500);
        set_deg_inc(86.0, 85.0, 1, .5, 5, ARMROT); //rotate to avoid jaw bracket touch tube top
        delay(500);
...
void motionsevenCtoHold()
{
       //set_deg_inc(110.0, 55.0, 1, 1, 10, JAW);  //Send JAW (JAW CLOSURE POSITION) to INTERMEDIATE position using incremental motion for safe holding
        set_deg_inc(140.0, 84.0, 1, 1, 10, JAWPIVOT);  //Send JAWPIVOT (JAW PIVOTING)to an INTERMEDIATE position using incremental motion for safe holding
        set_deg_inc(70.0, 68.0, 1, 1, 10, JAWROT);  //Send JAWROT (JAW ROTATION) to an INTERMEDIATE position using incremental motion for safe holding
...
void Demo()
{
	    ...
	    set_deg_inc(50.0,140.0,0,1,5,JAWPIVOT); //move from hold
	    set_deg_inc(100.0,92.0, 1 , 1, 10,ARMROT);//move from hold
	    set_deg_inc(150.0,70.0,1,1,5,JAWROT);//move from hold
	    grabtube(2); //release tube
	    delay(200);
		...
		delay (200);
		grabtube(3);
		set_deg_inc(0.0,150.0,1,1,5,JAWROT);  //Send JAW ROTATION to extreme position using incremental motion algorithm
		delay (200);
		set_deg_inc(150.0,0.0,0,1,5,JAWROT);  //Send JAW ROTATION to extreme position using incremental motion algorithm
		delay (200);
		...
		delay (200);
		grabtube(2);
		set_deg_inc(140.0, 130.0,1,1,5,JAWPIVOT); //back to holds
		set_deg_inc(70.0,0.0,0,1,5,JAWROT);//back to holds
...
void motionextremes()
  ...
  //JAW ROATATION MOTION
		...
		delay (200);
		set_deg_abs(70, JAWROT);  //Return to safe hold
		delay (200);
